    ...
    <DashboardLayout title="Dashboard">
      <div className="p-6 space-y-6">
            ...
            >
              <Card className="bg-dark-light border-gray-800">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium text-light">{stat.title}</CardTitle>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          >
            <Card className="h-fit bg-dark-light border-gray-800">
              <CardHeader>
                <CardTitle className="flex items-center justify-between text-light">
                  Recent Tasks
                  <Button variant="outline" size="sm" className="border-gray-600 text-slate-300 hover:text-white">View All</Button>
                </CardTitle>
                <CardDescription className="text-slate-400">Your latest battles with productivity</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {recentTasks.map((task) => (
                  <div key={task.id} className="flex items-center justify-between p-3 border border-gray-700 rounded-lg bg-dark/50">
                    <div className="flex items-center space-x-3">
                      <CheckCircle 
                        className={`h-5 w-5 ${task.completed ? 'text-green-400' : 'text-slate-500'}`}
                      />
                      <div>
                        <p className={`font-medium text-light ${task.completed ? 'line-through opacity-60' : ''}`}>
                          {task.title}
                        </p>
                        <p className="text-xs text-slate-400 italic">{task.roast}</p>
                      </div>
                    </div>
          ...
          >
            ...
            <Card className="bg-dark-light border-gray-800">
              <CardHeader>
                <CardTitle className="flex items-center text-light">
              ...
              <CardContent className="space-y-3">
                {upcomingEvents.map((event, index) => (
                  <div key={index} className="flex items-center space-x-3">
                    <Badge variant="outline" className="text-xs border-gray-600 text-slate-300">
                      {event.time}
                    </Badge>
                    <p className="text-sm text-light">{event.title}</p>
                  </div>
                ))}
                <Button variant="outline" size="sm" className="w-full mt-3 border-gray-600 text-slate-300 hover:text-white">
              ...
              <CardContent className="space-y-4">
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-light">Learn React</span>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-light">Get Fit</span>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-light">Side Project</span>
